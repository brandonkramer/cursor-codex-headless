...
export function registerCodexHeadlessTools(server: McpServer): void {
  server.registerTool(
    ...
    {
      description:
        "Read-only Codex review via codex exec --profile review --ephemeral --ignore-user-config (gpt-5.6-sol, xhigh; no global MCP). Use review_uncommitted/review_base for built-in diff review, or prompt for custom scope. Set structured=true for reviewer-verdict JSON schema.",
      inputSchema: {
        prompt: promptField,
        cwd: cwdField,
